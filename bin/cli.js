...
if (!jsonOutput) {
  console.log(`🔧 Compiled ${framework} → HTML (${(compiledHTML.length / 1024).toFixed(1)} KB)\n`);
    ...
    // Check if it's a project dependency (not our internal module)
    const isProjectDependency = missingModule && 
      !missingModule.includes('email-render-check') &&
      !missingModule.includes('cli.js') &&
      !missingModule.includes('compiler') &&
      !missingModule.includes('checker');
    ...
    if (jsonOutput) {
      console.log(JSON.stringify({
        ...
        file: fileArg,
        framework: framework,
        missingModule: isProjectDependency ? missingModule : null,
        message: isProjectDependency 
          ? `Compilation failed: Cannot find module '${missingModule}'. Install it in your project.`
          : `Compilation failed: ${stderr.split('\n')[0]}`,
        hint: isProjectDependency
          ? `Install the missing dependency: npm install ${missingModule}`
          : 'Check that your file exports a valid component with all dependencies installed.',
        summary: {
          totalProperties: 0,
          totalErrors: 0,
          totalWarnings: 0,
    ...
    console.error(`❌ Compilation failed for ${path.basename(fileArg)}`);
    
    if (isProjectDependency) {
      console.error(`   Missing dependency: ${missingModule}`);
      console.error(`   Install it: npm install ${missingModule}`);
    } else {
      console.error(`   ${stderr.split('\n')[0]}`);
      console.error(`   Make sure all dependencies are installed and the file is valid.`);
    }
